...
var state = sqliteTable("state", {
  key: text("key").primaryKey(),
  value: text("value").notNull(),
  updated_at: integer("updated_at").notNull()
});
var commands_queue = sqliteTable("commands_queue", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
// api/[[route]].ts
var app = new Hono2();
var ADMIN_DISCORD_ID = "1339570380943261697";
...
app.get("/api/auth/callback", async (c) => {
  ...
  try {
    ...
    const now = Math.floor(Date.now() / 1e3);
    await db.insert(state).values({ key: "oauth_user_data", value: JSON.stringify(userData), updated_at: now }).onConflictDoUpdate({ target: state.key, set: { value: JSON.stringify(userData), updated_at: now } }).run();
    await db.insert(state).values({ key: "oauth_connections", value: JSON.stringify(connectionsData), updated_at: now }).onConflictDoUpdate({ target: state.key, set: { value: JSON.stringify(connectionsData), updated_at: now } }).run();
    await db.insert(state).values({ key: "oauth_guilds", value: JSON.stringify(guildsData), updated_at: now }).onConflictDoUpdate({ target: state.key, set: { value: JSON.stringify(guildsData), updated_at: now } }).run();
  } catch (e) {
    console.error("Failed to save oauth data", e);
  }
  const token = await sign2(payload, c.env.JWT_SECRET || "fallback_secret_change_me");
...
app.get("/api/auth/me", async (c) => {
  ...
  try {
    const decoded = await verify2(token, c.env.JWT_SECRET || "fallback_secret_change_me", "HS256");
...
var dashboardAuth = /* @__PURE__ */ __name(async (c, next) => {
  const token = getCookie(c, "session");
...
var botAuth = /* @__PURE__ */ __name(async (c, next) => {
...
app.post("/api/push-state", botAuth, async (c) => {
  ...
  for (const item of body) {
    ...
    if (key && value !== void 0) {
      await db.insert(state).values({ key, value: JSON.stringify(value), updated_at: now }).onConflictDoUpdate({ target: state.key, set: { value: JSON.stringify(value), updated_at: now } }).run();
    }
  }
  return c.json({ success: true });
});
app.get("/api/poll-commands", botAuth, async (c) => {
  const db = drizzle(c.env.DB);
  const pending = await db.select().from(commands_queue).where(eq(commands_queue.status, "pending")).limit(10).all();
  if (pending.length > 0) {
    for (const cmd of pending) {
      await db.update(commands_queue).set({ status: "executing" }).where(eq(commands_queue.id, cmd.id)).run();
    }
  }
...
// ../.wrangler/tmp/pages-iciBaT/functionsRoutes-0.758008906129749.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
